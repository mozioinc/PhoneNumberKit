...
open class PhoneNumberTextField: UITextField, UITextFieldDelegate {
    ...
    public static func textField(_ textField: UITextField, shouldChangeCharactersInRange range: NSRange, replacementString string: String, partialFormatter: PartialFormatter) -> Bool {
        guard let text = textField.text else {
            return false
        }
        
        // allow delegate to intervene
        guard (textField as? PhoneNumberTextField)?._delegate?.textField!(textField, shouldChangeCharactersIn: range, replacementString: string) ?? true else {
            return false
        }
